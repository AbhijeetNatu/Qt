    ...
    : QGraphicsView(pParent) , m_oScreenSize(oScreenSize)
{
    ...
    setBackgroundBrush(QBrush(QImage(":/new/prefix1/Resources/SpaceInvadersBg.jpg")));
...
void CSpaceInvaders::Run()
{
    ...
    m_pCannon->setPos(m_oScreenSize.width()/2,m_oScreenSize.height() - gCannon.height());
...
void CSpaceInvaders::keyPressEvent(QKeyEvent *pEvent)
{
    ...
    switch (pEvent->key())
    {
        ...
        case Qt::Key_Right:
                if(m_pCannon->pos().x() + gCannon.width() < m_oScreenSize.width())
                    m_pCannon->setPos(m_pCannon->x()+20,m_pCannon->y());
                break;
